...
<body>
    ...
    <section>
        <div class="container mt-5">
            ...
            $userQuery = $connection->ShowTicket($conobj, "ticket");
            oci_execute($userQuery);
            $row = oci_fetch_assoc($userQuery);

            if (oci_num_rows($userQuery) > 0) {
                echo "<table><tr><th>TICKET_NO</th><th>TICKET_DATE</th><th>TICKET_DESCRIPTION</th><th>TICKET_TIME</th><th>Action</th></tr>";
                // output data of each row
                while ($row = oci_fetch_assoc($userQuery)) {
                    echo "<tr><td>" . $row["TICKET_NO"] . "</td><td>" . $row["TICKET_DATE"] . "</td><td>" . $row["TICKET_DESCRIPTION"] . "</td><td>" . $row["TICKET_TIME"] . "</td><td>" . "<a href='../views/ticket.php?TICKET_ID=" . $row["TICKET_ID"] . "'>View</a>" . "</td></tr>";
                }
                echo "</table>";
            } else {
                echo "0 results";
