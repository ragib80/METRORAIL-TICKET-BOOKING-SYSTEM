...
<body>
    ...
    <section>
        <div class="container mt-5">
            ...
            $userQuery = $connection->ShowLocation($conobj, "trainLocation");
            oci_execute($userQuery);
            // $row = oci_fetch_assoc($userQuery);
            // if (oci_num_rows($userQuery) > 0) {
            echo "<table><tr><th>LOCATION_ID</th><th>TRAIN_NO</th><th>TRAIN_LOCATION</th><th>TRAIN_DEPATURE</th><th>Action</th></tr>";
            ...
            while ($row = oci_fetch_array($userQuery, OCI_RETURN_NULLS + OCI_ASSOC)) {
                echo "<tr><td>" . $row["LOCATION_ID"] . "</td><td>" . $row["TRAIN_NO"] . "</td><td>" . $row["TRAIN_LOCATION"] .
                    "</td><td>" . $row["TRAIN_DEPATURE"] . "</td><td>" . "<a href='../views/ticket.php?TICKET_ID=" . $row["LOCATION_ID"] . "'>View</a>" . "</td></tr>";
            }
            echo "</table>";
